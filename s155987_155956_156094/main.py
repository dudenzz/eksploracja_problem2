...
def run_comparison():
    ...
    if fast_solution is not None:
        try:
            eclat_stats = benchmark_solution(fast_solution, "ECLAT C++ (Fast)", ITERATIONS, MIN_SUPPORT, MIN_CONFIDENCE)
            results.append(eclat_stats)
        except Exception as e:
            print(f"Błąd podczas testu ECLAT C++: {e}")
    else:
        print("fast_solution niedostępne — skompiluj moduł C++ (pip install -e .)")
